<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM mjsecbadge WHERE username = ?");
        $stmt->execute([$username]);
        $badgeExists = $stmt->fetchColumn();

        if ($badgeExists > 0) {
            $stmt = $pdo->prepare("SELECT badge_key FROM mjsecbadge WHERE username = ?");
            $stmt->execute([$username]);
        ...
        $stmt = $pdo->prepare("INSERT INTO mjsecbadge (username, badge_name, badge_key) VALUES (?, ?, ?)");
...
<head>
    <meta charset="UTF-8">
    <title>Create MJSEC Badge</title>
    <link rel="stylesheet" href="/static/css/generate_badge.css">
</head>
<body>
    <div class="container">
        <h1>Create MJSEC Badge</h1>
        <p>MJSEC 인증을 발급하시려면 비밀번호를 입력하세요.</p>
